...
function setLoading(prefix, on) {
  ...
  const label   = document.getElementById(prefix + '-btn-label');
...
async function runResumeAudit() {
  ...
  if (!resumeA)  { showError('r-results', 'Please paste the resume for Sarah Johnson.'); return; }
  if (!resumeB)  { showError('r-results', 'Please paste the resume for Syed Ahmed.'); return; }
...
function runResumeDemo() {
  renderScoreResults('r-results', 'RESUME',
    { overall:87, skills:89, experience:85, education:88, communication:86, justification:"Strong technical profile with clear leadership experience and well-structured presentation." },
    { overall:73, skills:75, experience:72, education:74, communication:71, justification:"Comparable qualifications noted, though language patterns and name-associated signals reduced overall score." },
    ['skills','experience','education','communication'],
    { skills:'Skills', experience:'Experience', education:'Education', communication:'Communication' }
  );
}
...
function runAudioDemo() {
  ...
  renderAudioResults('a-results', tA, tB,
    { overall:91, relevance:93, clarity:92, depth:90, communication:89, justification:"Confident, structured response with clear ownership of outcomes." },
    { overall:76, relevance:79, clarity:74, depth:77, communication:74, justification:"Similar technical content but conversational markers and syntax variations scored lower by language model." }
  );
}
...
async function runAudioAudit() {
  ...
  if (!fileA) { showError('a-results', 'Please upload an audio file for Sarah Johnson.'); return; }
  if (!fileB) { showError('a-results', 'Please upload an audio file for Syed Ahmed.'); return; }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// RENDER — AUDIO (transcripts + scores)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function renderAudioResults(areaId, transcriptA, transcriptB, scoreA, scoreB) {
  let html = `
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
    <div class="glass rounded-xl p-4">
      <p style="color:#3b82f6;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;margin-bottom:8px;">TRANSCRIPT — SARAH JOHNSON</p>
      <p style="color:#475569;font-size:0.82rem;line-height:1.7;">${esc(transcriptA)}</p>
    </div>
    <div class="glass rounded-xl p-4">
      <p style="color:#0891b2;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;margin-bottom:8px;">TRANSCRIPT — SYED AHMED</p>
      <p style="color:#475569;font-size:0.82rem;line-height:1.7;">${esc(transcriptB)}</p>
    </div>
  </div>`;
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function renderScoreResults(areaId, auditType, scoreA, scoreB, dims, dimLabels, append = false) {
  ...
  if (biasDetected) {
    html += `
    <div class="bias-banner p-5 mb-5">
    ...
    html += `
    <div class="clear-banner p-4 mb-5">
  ...
  const cards = [
    ...
    { name: 'Syed Ahmed',    score: scoreB, accent: '#0891b2', slot: 'CANDIDATE B' }
  ];

  cards.forEach(c => {
    html += `
    <div class="glass rounded-xl p-5 glass-hover" style="border-color:${c.accent}20;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:18px;">
        <div class="score-circle" style="border-color:${c.accent};background:${c.accent}14;color:${c.accent};">
          ${c.score.overall}
        </div>
        <div>
          <p style="color:#64748b;font-size:0.68rem;font-weight:700;letter-spacing:0.08em;">${c.slot}</p>
          <p style="color:#0f172a;font-weight:700;font-size:0.95rem;">${c.name}</p>
      ...
      <p style="color:#475569;font-size:0.78rem;margin-top:14px;line-height:1.6;font-style:italic;">"${esc(c.score.justification)}"</p>
    </div>`;
  });
  html += `</div>`;

  // Human review gate (bias only)
  if (biasDetected) {
...
function buildReviewGate(id, auditType, scoreA, scoreB, delta) {
  return `
  <div class="glass review-gate rounded-xl p-6" id="${id}-gate">
    ...
    <div style="margin-bottom:16px;">
      <label for="${id}-notes">Reviewer Notes</label>
      <textarea id="${id}-notes" rows="3" placeholder="Provide reasoning for your decision…"></textarea>
    </div>

    <div style="display:flex;gap:12px;">
      <button id="${id}-btn-reject"  class="btn btn-danger-outline" style="flex:1;" disabled
        onclick="submitDecision('${auditType}',${scoreA},${scoreB},${delta},'REJECTED','${id}')">
        ✕ Reject Candidates
      </button>
      <button id="${id}-btn-advance" class="btn btn-advance" style="flex:1;" disabled
...
function submitDecision(auditType, scoreA, scoreB, delta, decision, id) {
  ...
  const a    = Object.assign(document.createElement('a'), { href: url, download: `fairvoice-${logId}.json` });
  a.click();
...
function showError(areaId, msg) {
  document.getElementById(areaId).innerHTML = `<div class="error-box">⚠️ ${esc(msg)}</div>`;
}
